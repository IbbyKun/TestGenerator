...
export default function Publish() {
  ...
  return (
    <div className="flex flex-col py-9 pr-20 pl-8 text-2xl text-white bg-white max-md:px-5">
      ...
      <div className="shrink-0 mt-1.5 ml-0.5 bg-purple-700 border-purple-700 border-solid border-[5px] h-[5px] w-[226px] max-md:ml-2.5" />
      ...
      <div className="flex gap-5 justify-between self-center mt-6 w-full text-xl font-bold whitespace-nowrap max-w-[944px] max-md:flex-wrap max-md:max-w-full">
        <button
          onClick={handleClick}
          className="justify-center items-start px-10 py-4 bg-purple-700 rounded-xl max-md:px-5"
        >
          Edit
        </button>
        <Link
          to="/in"
          className="justify-center px-11 py-4 bg-purple-700 rounded-xl max-md:px-5"
        >
          Publish
        </Link>
      </div>
